            ...
            foreach (var item in details)
            {
                ...
                if (item.Quantity * request.Quantity > stock)
                {
                    return Result<string>
                        .Failure(item.Product!.Name + " ürünündenm üretim için yeterli miktar yok. Eksik miktar: " + ((item.Quantity * request.Quantity) - stock));
                }
                ...
                foreach (var depotId in depotIds)
                {
                    ...
                    {
                        stockMovement.NumberOfOutputs = item.Quantity * request.Quantity;
                    }
                    else
                    {
